...
class ObjectPoseServer(Node):
    ...
    def execute_callback(self, goal_handle):
        ...
        bounds_query = goal.target_obj_bounds
        result = GetTargetObjPose.Result()
        feedback = GetTargetObjPose.Feedback()

        if not self.objects:
            feedback.status_text = "No object metadata available yet."
            goal_handle.publish_feedback(feedback)
            self.get_logger().warn("No object metadata available.")
            goal_handle.abort()
            return result

        feedback.status_text = f"Searching for object with bounds {bounds_query}..."
        goal_handle.publish_feedback(feedback)

        found_obj = None
        for obj in self.objects:
            if self.bounds_match(obj.bounds, bounds_query):
                found_obj = obj
                break

        if found_obj:
            feedback.status_text = "Object found."
            goal_handle.publish_feedback(feedback)
            result.target_obj_found = 1
            result.target_obj_pose = found_obj.object_pose
            goal_handle.succeed()
            self.get_logger().info(
                f"Object match found (id={found_obj.id}); returning pose at "
                f"({found_obj.object_pose.pose.position.x:.3f}, "
                f"{found_obj.object_pose.pose.position.y:.3f}, "
                f"{found_obj.object_pose.pose.position.z:.3f})"
            )
        else:
            feedback.status_text = "No matching object found."
            goal_handle.publish_feedback(feedback)
            result.target_obj_found = 0
            goal_handle.abort()
            self.get_logger().warn("No matching object bounds found in cache.")
